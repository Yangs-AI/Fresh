...
def _datearea2submission_(data_frame: pandas.DataFrame):
    ...
    def group(time_type: str) -> tuple[list[str], list[bokeh.models.ColumnDataSource]]:
        partial_data_frame = data_frame.copy()
        partial_data_frame['_date_'] = partial_data_frame[time_type].dt.tz_convert('UTC').dt.date
        da2s = partial_data_frame.groupby(['_date_', '_area_']).size().reset_index(name='_count_')
        area_labels = da2s.groupby('_area_')['_count_'].sum().sort_values(ascending=False).index.tolist()
        ...
        for area_label in area_labels:
            d2s_area = da2s[da2s['_area_'] == area_label]
    ...
    fig = bokeh.plotting.figure(height=360, x_axis_type='datetime', title='Submission Count w.r.t. Date', tools='')
    fig.add_tools(
    ...
    palette = [bokeh.palettes.Category20[20][index % 20] for index in range(len(labels))]
    renderers = list()
    for index, label in enumerate(labels):
        renderer = fig.line('x', 'y', source=create_sources[index], line_width=2.0, color=palette[index], name=label, legend_label=f'{index}')
        ...
        bokeh.models.CustomJS(
            ...
            code="""
                ...
                for (let i = 0; i < renderers.length; i++) {
                    renderers[i].visible = active.has(i);
                }
            """
        )
    )
    ...
    available_time_types = ['_create_date_', '_finish_date_']
    selection = bokeh.models.Select(
        title='Time Type',
        value='_create_date_',
        options=[
            ('_create_date_', 'Created Date'),
            ('_finish_date_', 'Finished Date')
        ]
    )
    ...
    c_snap = snapshot(create_sources)
    f_snap = snapshot(finish_sources)

    selection.js_on_change(
        'value',
        bokeh.models.CustomJS(
            args=dict(renderers=renderers, c_snap=c_snap, f_snap=f_snap),
            code="""
                const useFinish = (cb_obj.value === '_finish_date_');
                const fromList = useFinish ? f_snap : c_snap;
                ...
                for (let i = 0; i < renderers.length; i++) {
                    ...
                    for (const k in from) {
                        const v = from[k];
                        nd[k] = (Array.isArray(v) || (v && v.buffer)) ? Array.from(v) : v;
                    }
                    const ds = renderers[i].data_source;
                    ds.data = nd;
                    ds.change.emit();
                }
            """
...
def _area2abstract_(data_frame: pandas.DataFrame):
    def get_abstract_word_count(abstract: str) -> int:
        words = textblob.Sentence(abstract).words
        return len(words)

    data_frame['_area_'] = data_frame['_area_'].fillna('Unknown').astype(str)
    data_frame['_abstract_'] = data_frame['_abstract_'].fillna('').astype(str)
    data_frame['_abstract_length_'] = data_frame['_abstract_'].apply(get_abstract_word_count)
    ...
    default_data_source = bokeh.models.ColumnDataSource(hist_dict['_all_'])
    fig = bokeh.plotting.figure(height=350, title='Histogram of Abstract', tools="pan,wheel_zoom,box_zoom,reset,save,hover")
    fig.quad(top='count', bottom=0, left='left', right='right', source=default_data_source, fill_alpha=0.6, line_color="#AE51C0")
    ...
    data_sources = {'_all_': default_data_source}
    for area in areas:
        data_sources[f'{area}'] = bokeh.models.ColumnDataSource(hist_dict[area])

    selection = bokeh.models.Select(title="Area", value="_all_", options=areas)
    selection.js_on_change(
        'value',
        bokeh.models.CustomJS(
            args=dict(show_src=default_data_source, all_srcs=data_sources),
            code="""
                const from = all_srcs[cb_obj.value].data;
                show_src.data = Object.assign({}, from);
                show_src.change.emit();
            """
        )
    )
...
def iclr(context: click.Context, year: int, topk: int, output_dirpath: pathlib.Path, online: bool, refetch: bool):
    ...
    if online:
        ...
        tabs = bokeh.models.Tabs(tabs=[
            bokeh.models.TabPanel(child=da2s_block, title="Submission Count w.r.t. Time Change"),
            bokeh.models.TabPanel(child=a2a_block,  title="Abstract Length"),
        ])
